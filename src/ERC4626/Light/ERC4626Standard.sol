...
abstract contract ERC4626Standard is ERC4626Abstract {
    ...
    public virtual {
        ...
        assertEq(
            sharesSeenByAlice,
            sharesSeenByBob,
            "Shares seen by Alice differs from shares seen by Bob after successful `convertToShares` call."
        );
    }
    ...
    public virtual {  
        ...
        assertEq(
            assetsSeenByAlice,
            assetsSeenByBob,
            "Assets seen by Alice differs from assets seen by Bob after successful `convertToAssets` call."
        );
    }
    ...
    public virtual {
        ...
        assertEq(
            maxDepositAlice,
            maxDepositBob,
            "`maxDeposit(alice) != maxDeposit(bob)` even though both balances of assets differ."
        );
    }
    ...
    public virtual {
        ...
        if (cut4626.maxDeposit(bob) != MAX_UINT256) {
            ...
            if (cut4626.maxDeposit(bob) > 0) {
                ...
                assertTrue(
                    callDeposit,
                    "Alice cannot deposit a number of assets that is lesser than `maxDeposit(bob)` for Bob."
                );
            } else {
    ...
    public virtual {
        ...
        (bool callDeposit, uint256 mintedShares) =
            tryCallerDepositAssetsToReceiverWithChecksAndApproval(alice, assets, bob);
	...
	public virtual {
        ...
        assertTrue(
            callDeposit,
            "Alice cannot deposit assets to Bob even though she has provided enough assets' allowance to the vault."
        );
    }
	...
	public virtual {
        ...
        assertFalse(
            callDeposit,
            "Alice can deposit assets to Bob even though she has not provided enough assets' allowance to the vault."
        );
    }
    ...
    public virtual {
        ...
        if (cut4626.maxWithdraw(alice) > 0) {
            ...
            assertTrue(
                callWithdraw,
                "Alice cannot withdraw an amount of assets that is lesser than `maxWithdraw(alice)` for Bob."
            );
        }
    }
    ...
    public virtual {
        ...
        conditionalSkip(
            !callWithdraw && withdrawnShares == 0, "Inconclusive test: Alice cannot withdraw assets for Carol."
        );
        // 3. Compare the values found in step 1 and 2
        assertApproxGeAbs(
    ...
    public virtual {
        ...
        assertApproxEqAbs(
            aliceShares - aliceSharesAfter,
            burnedShares,
            delta,
            "Alice's balance of shares does not decrease by the burned amount as expected (up to `delta`-approximation)."
        );
    }
    ...
    public virtual {
        ...
        (bool callWithdraw, uint256 burnedShares) =
            tryCallerWithdrawAssetsToReceiverFromOwnerWithChecksAndApproval(bob, assets, carol, alice);
        // 2a. Check that the withdraw call succeeded
        assertTrue(
            callWithdraw,
            "Bob cannot withdraw Alice's assets for Carol even though Alice has approved Bob enough allowance."
        );
        uint256 totalSupplyAfter = cut4626.totalSupply();
        // 2b. Check that the right amount of shares is burnt from the total supply of shares
        assertGt(totalSupplyBefore, totalSupplyAfter, "The total supply of the vault does not decrease as expected.");
        ...
        assertApproxEqAbs(
            aliceShares - cut4626.balanceOf(alice),
            burnedShares,
            delta,
            "Alice's balance of shares does not decrease by the burned amount as expected (up to `delta`-approximation)."
        );
    ...
    public virtual {
        ...
        assertEq(
            maxMintAlice, maxMintBob, "`maxMint(alice) != maxMint(bob)` even though both balances of assets differ."
        );
    }
	...
	public virtual {
        ...
        assertTrue(
            callMint,
            "Alice cannot mint shares to Bob even though she has provided enough assets' allowance to the vault."
        );
    }
	...
	public virtual {
        ...
        if (callMint) {
            ...
            assertLe(
                depositedAssets,
                approvedAssets,
                "Alice can mint shares to Bob even though she has not provided enough assets' allowance to the vault."
            );
        } else {
    ...
    public virtual {
        ...
        if (cut4626.maxRedeem(alice) > 0) {
            ...
            assertTrue(
                callRedeem, "Alice cannot redeem an amount of shares that is lesser than `maxRedeem(alice)` for Bob."
            );
        }
    }
    ...
    public virtual {
        ...
        assertTrue(
            callRedeem,
            "Bob cannot redeem Alice's shares for Carol even though Alice has approved Bob enough allowance."
        );
        uint256 totalSupplyAfter = cut4626.totalSupply();
        uint256 aliceSharesAfter = cut4626.balanceOf(alice);
        // 2b. Check that the right amount of shares is burnt from the total supply of shares
	...
	public virtual {
        ...
        assertTrue(
            callWithdraw,
            "Bob cannot withdraw assets from Alice's account for Carol even though he has enough allowance from Alice."
        );
    }
    ...
    public virtual {
        ...
        assertFalse(
            callWithdraw,
            "Bob can withdraw assets from Alice's account for Carol even though he does not have enough allowance from Alice."
        );
    }
	...
	public virtual {
        ...
        assertTrue(
            callRedeem,
            "Bob cannot redeem shares from Alice's account for Carol even though he has enough allowance from Alice."
        );
    }
    ...
    public virtual {
        ...
        assertFalse(
            callRedeem,
            "Bob can redeem shares from Alice's account for Carol even though he does not have enough allowance from Alice."
        );
    }
