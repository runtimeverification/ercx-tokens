...
abstract contract ERC4626Standard is ERC4626Abstract {
    ...
    ****************************/
    ...
    function testConvertToSharesNoVariationOnCaller(address user1, address user2, uint256 user1Assets, uint256 user2Assets, uint256 assets) 
    initializeAssetsTwoUniqueNonZeroAddresses(user1, user2, user1Assets, user2Assets) assetsOverflowRestriction(assets)
    public {
        vm.assume(assets > 0);
        ...
        assertEq(sharesSeenBy1, sharesSeenBy2, "Shares seen by dummy user 1 differs from shares seen by dummy user 2 after successful `convertToShares` call.");
    }
    ...
    ****************************/
    ...
    function testConvertToAssetsNoVariationOnCaller(address user1, address user2, uint256 user1Shares, uint256 user2Shares, uint256 shares) 
    initializeSharesTwoUniqueNonZeroAddresses(user1, user2, user1Shares, user2Shares) sharesOverflowRestriction(shares) 
    public {  
        vm.assume(shares > 0);   
        ...
        assertEq(assetsSeenBy1, assetsSeenBy2, "Assets seen by dummy user 1 differs from assets seen by dummy user 2 after successful `convertToAssets` call.");
    }
    ...
    ****************************/
    ...
    function testMaxDepositDoesNotRevert(address user)
    ...
    /// @custom:ercx-concerned-function maxDeposit
    function testMaxDepositNotRelyBalanceOfAssets(address user1, address user2, uint256 user1Assets, uint256 user2Assets)
    ...
    public {
        ...
        assertEq(maxDeposit1, maxDeposit2, "`maxDeposit(user1) != maxDeposit(user2)` even though both balances of assets differ.");
    } 
    ...
    /// @custom:ercx-concerned-function maxDeposit
    function testMaxDepositNotHigherThanActualMax(address depositor, address receiver, uint256 assets, uint256 depositorAssets)
    isNotZeroAddress(receiver) initializeAssetsOneNonZeroAddress(depositor, depositorAssets) assetsOverflowRestriction(assets) 
    public {
        ...
        if (cut4626.maxDeposit(receiver) != MAX_UINT256) {
            ...
            if (cut4626.maxDeposit(receiver) > 0) {
                ...
                assertTrue(callDeposit, "depositor cannot deposit a number of assets that is lesser than `maxDeposit(receiver)` for receiver.");
            }
            else {
    ...
    ****************************/
    ...
    function testPreviewDepositSameOrLessThanDeposit(address depositor, address receiver, uint256 assets, uint256 depositorAssets)
    isNotZeroAddress(receiver) initializeAssetsOneNonZeroAddress(depositor, depositorAssets) assetsOverflowRestriction(assets)
    public {
        vm.assume(assets > 0);
        vm.assume(assets <= depositorAssets);
        ...
        (bool callDeposit, uint256 mintedShares) = tryCallerDepositAssetsToReceiverWithChecksAndApproval(depositor, assets, receiver);
        // Skip the test if the deposit call failed and mintedShares == 0
        conditionalSkip(!callDeposit && mintedShares == 0, "Inconclusive test: depositor cannot deposit assets for receiver.");
    ...
    ****************************/
    ...
    public {
        ...
        conditionalSkip(!callDeposit, "Inconclusive test: depositor cannot deposit positive amount of assets to receiver.");
    ...
    /// @custom:ercx-concerned-function deposit
    function testDepositSupportsEIP20ApproveTransferFromAssets(address depositor, address receiver, uint256 depositorAssets, uint256 assets)
	isNotZeroAddress(receiver) initializeAssetsOneNonZeroAddress(depositor, depositorAssets) assetsOverflowRestriction(assets)
	public {
        ...
        assertTrue(callDeposit, "depositor cannot deposit assets to receiver even though she has provided enough assets' allowance to the vault.");
	}
    ...
    /// @custom:ercx-categories deposit, assets
    /// @custom:ercx-concerned-function deposit
    function testDepositFailsIfInsufficientAssetsAllowanceToVault(address depositor, address receiver, uint256 depositorAssets, uint256 assets)
	isNotZeroAddress(receiver) initializeAssetsOneNonZeroAddress(depositor, depositorAssets) assetsOverflowRestriction(depositorAssets)
	public {
        vm.assume(assets > 0);
        vm.assume(depositorAssets > 0);
        vm.assume(assets < depositorAssets);
        ...
        assertFalse(callDeposit, "depositor can deposit assets to receiver even though she has not provided enough assets' allowance to the vault.");
	}
    ...
    ****************************/
    ...
    function testMaxWithdrawNotHigherThanActualMax(address withdrawer, address receiver, uint256 assets, uint256 withdrawerShares)
    isNotZeroAddress(receiver) initializeSharesOneNonZeroAddress(withdrawer, withdrawerShares) assetsOverflowRestriction(assets) sharesOverflowRestriction(withdrawerShares) 
    public {
        ...
        if (cut4626.maxWithdraw(withdrawer) > 0) {
            ...
            assertTrue(callWithdraw, "withdrawer cannot withdraw an amount of assets that is lesser than `maxWithdraw(withdrawer)` for receiver.");
        }
    }
    ...
    /// @custom:ercx-concerned-function maxWithdraw
    function testMaxWithdrawDoesNotRevert(address user)
    isNotZeroAddress(user) 
    public {
    ...
    ****************************/
    ...
    function testPreviewWithdrawSameOrMoreThanWithdraw(address withdrawer, address receiver, uint256 assets, uint256 withdrawerShares)
    isNotZeroAddress(receiver) initializeSharesOneNonZeroAddress(withdrawer, withdrawerShares) assetsOverflowRestriction(assets) sharesOverflowRestriction(withdrawerShares)
    public {
        vm.assume(withdrawerShares > 0);
        vm.assume(assets > 0);
        ...
        (bool callWithdraw, uint256 withdrawnShares) = tryOwnerWithdrawAssetsToReceiverWithChecks(withdrawer, assets, receiver);
        // Skip the test if the withdraw call failed and withdrawnShares == 0
        conditionalSkip(!callWithdraw && withdrawnShares == 0, "Inconclusive test: withdrawer cannot withdraw assets for receiver.");
        // 3. Compare the values found in step 1 and 2
    ...
    ****************************/
    ...
    function testWithdrawEmitWithdrawEvent(address withdrawer, address receiver, uint256 assets, uint256 withdrawerShares)
    isNotZeroAddress(receiver) initializeSharesOneNonZeroAddress(withdrawer, withdrawerShares) assetsOverflowRestriction(assets) sharesOverflowRestriction(withdrawerShares)
    public {
        vm.assume(withdrawerShares > 0);
        vm.assume(assets > 0);
        ...
        conditionalSkip(!callWithdraw, "Inconclusive test: withdrawer cannot withdraw positive amount of assets to receiver.");
    ...
    /// @custom:ercx-categories withdraw, shares
    /// @custom:ercx-concerned-function withdraw
    function testWithdrawSupportsBurnSharesFromOwnerWhereOwnerIsMsgSender(address withdrawer, address receiver, uint256 assets, uint256 withdrawerShares)
    initializeSharesOneNonZeroAddress(withdrawer, withdrawerShares) assetsOverflowRestriction(assets) sharesOverflowRestriction(withdrawerShares)
    public {
        ...
        (bool callWithdraw, uint256 burnedShares) = tryOwnerWithdrawAssetsToReceiverWithChecks(withdrawer, assets, receiver);
        ...
        assertGt(withdrawerShares, withdrawerSharesAfter, "withdrawer's balance of shares does not decrease as expected.");
        assertApproxEqAbs(withdrawerShares - withdrawerSharesAfter, burnedShares, delta, "withdrawer's balance of shares does not decrease by the burned amount as expected (up to `delta`-approximation).");        
    }
    ...
    /// @custom:ercx-categories withdraw, shares
    /// @custom:ercx-concerned-function withdraw
    function testWithdrawSupportsBurnSharesFromOwnerWhereOwnerApprovesMsgSender(address owner, address withdrawer, address receiver, uint256 assets, uint256 ownerShares)
    initializeSharesOneNonZeroAddress(owner, ownerShares) assetsOverflowRestriction(assets) sharesOverflowRestriction(ownerShares)
    public {
        ...
        (bool callWithdraw, uint256 burnedShares) = tryCallerWithdrawAssetsToReceiverFromOwnerWithChecksAndApproval(withdrawer, assets, receiver, owner);
        // 2a. Check that the withdraw call succeeded
        assertTrue(callWithdraw, "withdrawer cannot withdraw owner's assets for receiver even though owner has approved withdrawer enough allowance.");
        // 2b. Check that the right amount of shares is burnt from the total supply of shares
        assertGt(totalSupplyBefore, cut4626.totalSupply(), "The total supply of the vault does not decrease as expected.");            
        assertApproxEqAbs(totalSupplyBefore - cut4626.totalSupply(), burnedShares, delta, "The total supply of the vault does not decrease by the burned amount as expected (up to `delta`-approximation).");
        ...
        assertApproxEqAbs(ownerShares - cut4626.balanceOf(owner), burnedShares, delta, "owner's balance of shares does not decrease by the burned amount as expected (up to `delta`-approximation).");
    }
    ...
    ****************************/
    ...
    function testMaxMintDoesNotRevert(address user)
    public isNotZeroAddress(user) {
    ...
    isNotZeroAddress(receiver) initializeAssetsOneNonZeroAddress(minter, minterAssets) sharesOverflowRestriction(shares) 
    public {
        ...
        if (cut4626.maxMint(receiver) != MAX_UINT256) {
            ...
            if (cut4626.maxMint(receiver) > 0) {
                ...
                assertTrue(callMint, "minter cannot mint a number of shares that is lesser than `maxMint(receiver)` for receiver.");
            }
            else {
    ...
    ****************************/
    ...
    public {
        ...
        (bool callMint, uint256 mintedAssets) = tryCallerMintSharesToReceiverWithChecksAndApproval(minter, shares, receiver);
    ...
    ****************************/
    ...
    /// @custom:ercx-concerned-function mint
    function testMintSupportsEIP20ApproveTransferFromAssets(address minter, address receiver, uint256 minterAssets, uint256 shares)
	isNotZeroAddress(receiver) initializeAssetsOneNonZeroAddress(minter, minterAssets) sharesOverflowRestriction(shares)
	public {
        vm.assume(minterAssets > 0);
        vm.assume(shares > 0);
        ...
        assertTrue(callMint, "minter cannot mint shares to receiver even though she has provided enough assets' allowance to the vault.");
	}
    ...
    /// @custom:ercx-categories mint, assets
    /// @custom:ercx-concerned-function mint
    function testMintFailsIfInsufficientAssetsAllowanceToVault(address minter, address receiver, uint256 minterAssets, uint256 shares, uint256 approvedAssets)
	isNotZeroAddress(receiver) initializeAssetsOneNonZeroAddress(minter, minterAssets) sharesOverflowRestriction(shares)
	public {
        vm.assume(minterAssets > 0);
        vm.assume(shares > 0);
        ...
        if (callMint) {
            ...
            assertLe(depositedAssets, approvedAssets, "minter can mint shares to receiver even though she has not provided enough assets' allowance to the vault.");
        }
        else {
            // 3b. Check that the mint call failed otherwise
            assertFalse(callMint, "minter can mint shares to receiver even though she has not provided enough assets' allowance to the vault.");
        }
	}
    ...
    ****************************/
    ...
    function testMaxRedeemNotHigherThanActualMax(address redeemer, address receiver, uint256 shares, uint256 redeemerShares) 
    isNotZeroAddress(receiver) initializeSharesOneNonZeroAddress(redeemer, redeemerShares) sharesOverflowRestriction(shares) 
    public {
        ...
        if (cut4626.maxRedeem(redeemer) > 0) {
            ...
            assertTrue(callRedeem, "redeemer cannot redeem an amount of shares that is lesser than `maxRedeem(redeemer)` for receiver.");
        }
    }
    ...
    /// @custom:ercx-concerned-function maxRedeem
    function testMaxRedeemDoesNotRevert(address user)
    isNotZeroAddress(user)
    public {
    ...
    ****************************/
    ...
    function testPreviewRedeemSameOrLessThanRedeem(address redeemer, address receiver, uint256 shares, uint256 redeemerShares)
    isNotZeroAddress(receiver) initializeSharesOneNonZeroAddress(redeemer, redeemerShares) sharesOverflowRestriction(shares)
    public {
        vm.assume(shares > 0);
        ...
        conditionalSkip(!callRedeem && redeemedAssets == 0, "Inconclusive test: redeemer cannot redeem shares for receiver.");
        // 3. Compare the values found in step 1 and 2
    ...
    ****************************/
    ...
    /// @custom:ercx-categories redeem, shares
    /// @custom:ercx-concerned-function redeem
    function testRedeemSupportsBurnSharesFromOwnerWhereOwnerIsMsgSender(address redeemer, address receiver, uint256 shares, uint256 redeemerShares)
    isNotZeroAddress(receiver) initializeSharesOneNonZeroAddress(redeemer, redeemerShares) sharesOverflowRestriction(shares)
    public {
        uint256 totalSupplyBefore = cut4626.totalSupply();
        vm.assume(shares > 0);
        ...
        assertApproxEqAbs(redeemerShares - redeemerSharesAfter, shares, delta, "redeemer's balance of shares does not decrease by the burned amount as expected (up to `delta`-approximation).");
    }
    ...
    /// @custom:ercx-categories redeem, shares
    /// @custom:ercx-concerned-function redeem
    function testRedeemSupportsBurnSharesFromOwnerWhereOwnerApprovesMsgSender(address owner, address redeemer, address receiver, uint256 shares, uint256 ownerShares)
    unique2NonZeroAddresses(owner, redeemer) initializeSharesOneNonZeroAddress(owner, ownerShares) sharesOverflowRestriction(shares)
    public {
        vm.assume(receiver != address(0x0)); // Did not use `isNotZeroAddress` modifier as stack too deep
        uint256 totalSupplyBefore = cut4626.totalSupply();
        vm.assume(shares > 0);
        ...
        (bool callRedeem, ) = tryCallerRedeemSharesToReceiverFromOwnerWithChecksAndApproval(redeemer, shares, receiver, owner);
        // 2a. Check that the redeem call succeeded
        assertTrue(callRedeem, "redeemer cannot redeem owner's shares for receiver even though owner has approved redeemer enough allowance.");
        uint256 totalSupplyAfter = cut4626.totalSupply();
        ...
        assertApproxEqAbs(ownerShares - ownerSharesAfter, shares, delta, "owner's balance of shares does not decrease by the burned amount as expected (up to `delta`-approximation).");
    }
    ...
    ****************************/   
    ...
    /// @custom:ercx-concerned-function withdraw
    function testWithdrawSenderCanSpendBelowSharesAllowance(address owner, address withdrawer, address receiver, uint256 assets, uint256 ownerShares)
	initializeSharesOneNonZeroAddress(owner, ownerShares) assetsOverflowRestriction(assets) sharesOverflowRestriction(ownerShares)
	public {
        vm.assume(withdrawer != address(0x0)); // Did not use `isNotZeroAddress` modifier as stack too deep
        vm.assume(receiver != address(0x0)); // Did not use `isNotZeroAddress` modifier as stack too deep
        ...
        assertTrue(callWithdraw, "withdrawer cannot withdraw assets from owner's account for receiver even though he has enough allowance from owner.");
	}
    ...
    /// @custom:ercx-categories shares, assets, withdraw, allowance
    /// @custom:ercx-concerned-function withdraw
    function testWithdrawSenderCannotSpendAboveSharesAllowance(address owner, address withdrawer, address receiver, uint256 assets, uint256 approvedShares, uint256 ownerShares)
    initializeSharesOneNonZeroAddress(owner, ownerShares) assetsOverflowRestriction(assets) sharesOverflowRestriction(ownerShares)
    public {
        vm.assume(withdrawer != address(0x0)); // Did not use `isNotZeroAddress` modifier as stack too deep
        vm.assume(receiver != address(0x0)); // Did not use `isNotZeroAddress` modifier as stack too deep
        ...
        assertFalse(callWithdraw, "withdrawer can withdraw assets from owner's account for withdrawer even though he does not have enough allowance from owner.");
    }
    ...
    ****************************/
    ...
    /// @custom:ercx-concerned-function redeem
	function testRedeemSenderCanSpendBelowSharesAllowance(address owner, address redeemer, address receiver, uint256 shares, uint256 ownerShares)
	initializeSharesOneNonZeroAddress(owner, ownerShares) sharesOverflowRestriction(shares)
	public {
        ...
        assertTrue(callRedeem, "redeemer cannot redeem shares from owner's account for receiver even though he has enough allowance from owner.");
	}
    ...
    /// @custom:ercx-categories shares, redeem, allowance
    /// @custom:ercx-concerned-function redeem
    function testRedeemSenderCannotSpendAboveSharesAllowance(address owner, address redeemer, address receiver, uint256 shares, uint256 ownerShares)
    initializeSharesOneNonZeroAddress(owner, ownerShares) sharesOverflowRestriction(shares)
    public {
        vm.assume(redeemer != address(0x0)); // Did not use `isNotZeroAddress` modifier as stack too deep
        vm.assume(receiver != address(0x0)); // Did not use `isNotZeroAddress` modifier as stack too deep
        ...
        assertFalse(callRedeem, "redeemer can redeem shares from owner's account for receiver even though he does not have enough allowance from owner.");
    }
