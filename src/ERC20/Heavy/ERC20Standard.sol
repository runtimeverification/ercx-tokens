...
abstract contract ERC20Standard is ERC20Abstract {
    ...
    ****************************/
    ...
    external initializeStateOneUserGeneralAddress(user, balance) isNotZeroAddress(user) {
        assertEq(cut.balanceOf(user), balance, "The value of balanceOf(user) does not equate the amount of tokens given to him.");
    }
    ...
    ****************************/
    ...
    external initializeStateOneUserGeneralAddress(tokenSender, balance) isNotZeroAddress(tokenReceiver) unique2Addresses(tokenSender, tokenReceiver) {

        assertSuccess(_tryCustomerTransfer(tokenSender, tokenReceiver, 0), "A user failed to transfer zero amount to another user.");
    }
    ...
    external initializeStateOneUserGeneralAddress(user, balance) {
    ...
    /// @custom:ercx-concerned-function transfer
    function testPositiveTransferEventEmission(uint256 amount, address tokenSender, uint256 balanceSender, address tokenReceiver, uint256 balanceReceiver) 
    ...
    ****************************/
    ...
    function testZeroTransferFromByOtherEmitsEvent(address transferInitiator, address tokenSender, uint256 balance, address tokenReceiver) 
    external initializeStateOneUserGeneralAddress(tokenSender, balance) unique3Addresses(transferInitiator, tokenSender, tokenReceiver)
    isNotZeroAddress(transferInitiator) isNotZeroAddress(tokenReceiver) {
        vm.expectEmit(true, true, true, true);
        emit Transfer(tokenSender, tokenReceiver, 0);
    ...
    /// @custom:ercx-categories transferFrom, zero amount, event
    /// @custom:ercx-concerned-function transferFrom
    function testZeroTransferFromByOtherToSameAccountEmitsEvent(address transferInitiator, address tokenSenderReceiver, uint256 balance)
    external initializeStateOneUserGeneralAddress(tokenSenderReceiver, balance) unique2Addresses(transferInitiator, tokenSenderReceiver)
    isNotZeroAddress(transferInitiator) isNotZeroAddress(tokenSenderReceiver) {
        vm.expectEmit(true, true, true, true);
    ...
    external initializeStateOneUserGeneralAddress(tokenSenderReceiver, balance) {
        vm.expectEmit(true, true, true, true);
        emit Transfer(tokenSenderReceiver, tokenSenderReceiver, 0);
    ...
    external initializeStateOneUserGeneralAddress(tokenSender, balance) unique2Addresses(tokenSender, tokenReceiver) isNotZeroAddress(tokenReceiver) {
        vm.expectEmit(true, true, true, true);
        emit Transfer(tokenSender, tokenReceiver, 0);
    ...
    /// @custom:ercx-concerned-function transferFrom
    function testZeroTransferFromByOtherFromAndToSameAccountPossible(address transferInitiator, address tokenSenderReceiver, uint256 balance)
    external initializeStateOneUserGeneralAddress(tokenSenderReceiver, balance)
    unique2Addresses(transferInitiator, tokenSenderReceiver)
    isNotZeroAddress(transferInitiator) isNotZeroAddress(tokenSenderReceiver) {
        assertSuccess(_tryCustomerTransferFrom(transferInitiator, tokenSenderReceiver, tokenSenderReceiver, 0), "A user failed to transferFrom zero amount from an account to the same account.");
    }
    ...
    /// @custom:ercx-categories transferFrom, zero amount
    /// @custom:ercx-concerned-function transferFrom
    function testZeroTransferFromByOtherPossibleFromAndToDifferentAccounts(address transferInitiator, address tokenSender, uint256 balance, address tokenReceiver)
    external initializeStateOneUserGeneralAddress(tokenSender, balance)
    unique3Addresses(transferInitiator, tokenSender, tokenReceiver)
    isNotZeroAddress(transferInitiator) isNotZeroAddress(tokenReceiver) {
        assertSuccess(_tryCustomerTransferFrom(transferInitiator, tokenSender, tokenReceiver, 0), "A user failed to transferFrom zero amount from an account to another account.");
    }
    ...
    external initializeStateOneUserGeneralAddress(user, balance) isNotZeroAddress(transferInitiator) unique2Addresses(transferInitiator, user) {
        assertSuccess(_tryCustomerTransferFrom(transferInitiator, user, user, 0), "A user failed to transferFrom zero amount from some other account to this other account.");
    }
    ...
    external initializeStateOneUserGeneralAddress(tokenSender, balance) isNotZeroAddress(tokenReceiver){
        assertSuccess(_tryCustomerTransferFrom(tokenSender, tokenSender, tokenReceiver, 0), "A user failed to transferFrom zero amount from her account to another user.");
    }
    ...
    /// @custom:ercx-concerned-function transferFrom
    function testPositiveTransferFromEventEmission(uint256 amount, address tokenSender, uint256 balance, address transferInitiator, address tokenReceiver) 
    external initializeStateOneUserGeneralAddress(tokenSender, balance)
    isNotZeroAddress(transferInitiator) isNotZeroAddress(tokenReceiver) {
        vm.assume(amount > 0);
        vm.assume(amount <= balance);
        CallResult memory callResultApprove = _tryCustomerApprove(tokenSender, transferInitiator, amount);
        conditionalSkip(!callResultApprove.success, "Inconclusive test: tokenSender failed to approve transferInitiator.");
        vm.expectEmit(true, true, true, true);
        emit Transfer(tokenSender, tokenReceiver, amount);
        CallResult memory callResultTransferFrom = _tryCustomerTransferFrom(transferInitiator, tokenSender, tokenReceiver, amount);
        conditionalSkip(!callResultTransferFrom.success, "Inconclusive test: transferInitiator failed to call `transferFrom` from tokenSender to tokenReceiver.");
    }
    ...
    ****************************/
    ...
    external isNotZeroAddress(tokenApprover) isNotZeroAddress(tokenApprovee) {
        vm.expectEmit(true, true, true, true);
    ...
    function testPositiveApproveAllowsZeroTransferFrom(address tokenApprover, address tokenApprovee, address tokenReceiver, uint256 approvedAmount, uint256 balance) 
    external initializeStateOneUserGeneralAddress(tokenApprover, balance) {
        internalTestPositiveApproveAllowsTransferFrom(tokenApprover, tokenApprovee, tokenReceiver, approvedAmount, 0, balance);
    }
    ...
    /// @custom:ercx-concerned-function approve, transferFrom
    function testPositiveApproveAllowsPositiveTransferFrom(address tokenApprover, address tokenApprovee, address tokenReceiver, uint256 approvedAmount, uint256 transferAmount, uint256 balance) 
    external {
        vm.assume(transferAmount > 0);
        internalTestPositiveApproveAllowsTransferFrom(tokenApprover, tokenApprovee, tokenReceiver, approvedAmount, transferAmount, balance);
    }
    ...
    function internalTestPositiveApproveAllowsTransferFrom(address tokenApprover, address tokenApprovee, address tokenReceiver, uint256 approveAmount, uint256 transferAmount, uint256 balance) 
    internal initializeStateOneUserGeneralAddress(tokenApprover, balance)
    isNotZeroAddress(tokenApprovee) isNotZeroAddress(tokenReceiver) {
        ...
        assertSuccess(_tryCustomerTransferFrom(tokenApprovee, tokenApprover, tokenReceiver, transferAmount), "The token approvee failed to transferFrom from the token approver to some other account account.");
    }
    ...
    ****************************/
    ...
    /// @custom:ercx-concerned-function approve
    function testPositiveApproveLeadsToAllowance(address tokenApprover, address tokenApprovee, uint256 amount) external {
        vm.assume(amount > 0);
    ...
    function _internalTestApproveLeadsToAllowance(address tokenApprover, address tokenApprovee, uint256 amount) internal {
        CallResult memory callApprove = _tryCustomerApprove(tokenApprover, tokenApprovee, amount);
        conditionalSkip(!callApprove.success, "Inconclusive test: tokenApprover failed to approve tokenApprovee.");
        assertEq(cut.allowance(tokenApprover, tokenApprovee), amount, "The value of allowance(tokenApprover, tokenApprovee) does not equate the value approved by tokenApprover to tokenApprovee.");
    }
