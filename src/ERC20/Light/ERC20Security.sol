...
abstract contract ERC20Security is ERC20Abstract {
    ...
    *****************************/
    ...
    function testAddressZeroHasNoToken() 
    external {
    ...
    function testZeroTransferFromToZeroShouldRevert(uint256 balance1, uint256 balance2) 
    external {
    ...
    /// @custom:ercx-concerned-function transferFrom
    function testPositiveTransferFromToZeroShouldRevert(uint256 amount, uint256 balance1, uint256 balance2) 
    external {
        vm.assume(amount > 0);
    ...
    internal initializeStateTwoUsers(balance1, balance2) {
        ...
        assertFail(_tryBobTransferFrom(alice, address(0x0), amount), "Bob was able to transfer from Alice to the zero address.");
    }
    ...
    /// @custom:ercx-categories transfer, zero address
    /// @custom:ercx-concerned-function transfer
    function testZeroAddressCannotTransferPositiveAmount(uint256 amount)
    external {
        vm.assume(amount > 0);
        vm.assume(amount < MAX_UINT256 - cut.totalSupply());
        _dealERC20Token(address(cut), address(0x0), amount);
        assertFail(_tryCustomerTransfer(address(0x0), alice, amount), "A transfer with the zero address as token sender was possible.");
    }
    ...
    function testZeroAddressCannotApprovePositiveAmount(uint256 approveAmount)
    external {
        vm.assume(approveAmount > 0);
        assertFail(_tryCustomerApprove(address(0x0), alice, approveAmount), "An approve with the zero address as token sender was possible.");
    }
    ...
    /// @custom:ercx-categories approval, zero address
    /// @custom:ercx-concerned-function approve
    function testCannotApprovePositiveAmountToZeroAddress(uint256 approveAmount)
    external {
        vm.assume(approveAmount > 0);
    ...
    ****************************/
    ...
    /// @custom:ercx-concerned-function transfer
    function testZeroSelfTransfer(uint256 balance1, uint256 balance2) 
    external {
    ...
    /// @custom:ercx-concerned-function transfer
    function testPositiveSelfTransfer(uint256 amount, uint256 balance1, uint256 balance2) 
    external {
        vm.assume(amount > 0);
    ...
    ****************************/
    ...
    /// @custom:ercx-categories transfer, zero amount, balance
    /// @custom:ercx-concerned-function transfer
    function testZeroTotalTransferToOther(uint256 balance2) 
    external {
    ...
    /// @custom:ercx-concerned-function transfer
    function testPositiveTotalTransferToOther(uint256 balance1, uint256 balance2) 
    external {
    ...
    internal initializeStateTwoUsers(balance1, balance2) {
        ...
        assertEq(cut.balanceOf(carol), carolInitialBalance + aliceInitialBalance, "Carol's balance has not been augmented by the amount of tokens sent to her.");
    }
    ...
    /// @custom:ercx-concerned-function transferFrom
    function testZeroTotalTransferFromToOther(uint256 balance2) 
    external {
    ...
    /// @custom:ercx-concerned-function transferFrom
    function testPositiveTotalTransferFromToOther(uint256 balance1, uint256 balance2) 
    external {
        vm.assume(balance1 > 0);
    ...
    ****************************/
    ...
    /// @custom:ercx-concerned-function transfer
    function testZeroMultipleTransfer(uint256 balance1, uint256 balance2) 
    external {
    ...
    /// @custom:ercx-categories transfer, balance
    /// @custom:ercx-concerned-function transfer
    function testPositiveMultipleTransfer(uint256 amount, uint256 balance1, uint256 balance2) 
    external {
        vm.assume(amount > 0);
    ...
    /// @custom:ercx-concerned-function transferFrom
    function testZeroMultipleTransferFrom(uint256 balance1, uint256 balance2) 
    external {
    ...
    /// @custom:ercx-concerned-function transferFrom
    function testPositiveMultipleTransferFrom(uint256 amount, uint256 balance1, uint256 balance2) 
    external {
        vm.assume(amount > 0);
    ...
    /// @custom:ercx-concerned-function transferFrom
    function testMultipleTransferFromCannotExceedAllowance(uint256 amount1, uint256 amount2, uint256 balance1, uint256 balance2) 
    external initializeStateTwoUsers(balance1, balance2) {
        vm.assume(amount1 > 0);
        ...
        conditionalSkip(!callResultTransferFrom.success, "Inconclusive test: Bob could not transferFrom his initial allowance.");
        assertFail(_tryBobTransferFrom(alice, carol,amount2), "Bob could transfer from Alice even if his allowance reached zero.");
    }
    ...
    ****************************/
    ...
    /// @custom:ercx-concerned-function approve
    function testCorrectOverwriteApproveZeroToZero()
    external {
        ...
        assertEq(cut.allowance(alice, bob), 0, "Allowance is not set to the right amount after the second `approve` is called.");
    }
    ...
    /// @custom:ercx-categories approval, zero amount
    /// @custom:ercx-concerned-function approve
    function testCorrectOverwriteApproveZeroToPositive(uint256 approveAmount2)
    external {
        // to make sure the allowance of Alice for bob is reset to 0
        if (cut.allowance(alice, bob) != 0) {
            assertSuccess(_tryAliceApprove(bob, 0));
        ...
        assertEq(cut.allowance(alice, bob), approveAmount2, "Allowance is not set to the right amount after the second `approve` is called.");
    }
    ...
    /// @custom:ercx-categories approval, zero amount
    /// @custom:ercx-concerned-function approve
    function testCorrectOverwriteApprovePositiveToZero(uint256 approveAmount1)
    external {
        // to make sure the allowance of Alice for bob is reset to 0
        if (cut.allowance(alice, bob) != 0) {
            assertSuccess(_tryAliceApprove(bob, 0));
    ...
    /// @custom:ercx-concerned-function approve
    function testCorrectOverwriteApprovePositiveToPositive(uint256 approveAmount1, uint256 approveAmount2)
    external {
        // to make sure the allowance of Alice for bob is reset to 0
        if (cut.allowance(alice, bob) != 0) {
            assertSuccess(_tryAliceApprove(bob, 0));
        ...
        if (secondApproveResult.success && secondApproveResult.optionalReturn != OptionalReturn.RETURN_FALSE) {
            assertEq(cut.allowance(alice, bob), approveAmount2, "Alice's allowance to Bob is not set to the right amount after the second `approve` is called.");
        }
        else {
    ...
    ****************************/
    ...
    /// @custom:ercx-categories approval, zero amount
    /// @custom:ercx-concerned-function approve
    function testZeroSelfApprove(uint256 balance1, uint256 balance2) 
    external {
    ...
    /// @custom:ercx-categories approval
    /// @custom:ercx-concerned-function approve
    function testPositiveSelfApprove(uint256 amount, uint256 balance1, uint256 balance2) 
    external {
        vm.assume(amount > 0);
    ...
    function testZeroSelfApproveTransferFrom(uint256 balance1, uint256 balance2) 
    external {
    ...
    /// @custom:ercx-concerned-function approve, transferFrom
    function testPositiveSelfApproveTransferFrom(uint256 amount, uint256 balance1, uint256 balance2) 
    external {
        vm.assume(amount > 0);
    ...
    internal initializeStateTwoUsers(balance1, balance2) {
        ...
        assertSuccess(_tryAliceTransferFrom(alice, bob, amount), "Alice could not transfer from her account after self approving herself.");
    }
    ...
    ****************************/
    ...
    /// @custom:ercx-categories transfer, fees
    /// @custom:ercx-concerned-function transfer
    function testFeeTakingTransferPresent(uint256 amount, uint256 balance1, uint256 balance2) 
    external {
    ...
    /// @custom:ercx-categories transfer, fees
    /// @custom:ercx-concerned-function transfer
    function testFeeTakingTransferPotential(uint256[2] memory feeAmounts, uint256 amount, uint256 balance1, uint256 balance2) 
    ...
    internal initializeStateTwoUsers(balance1, balance2) {
        ...
        if (callResultTransfer.success) {
            assertEq(preAliceBalance - amount, cut.balanceOf(alice), "Alice's balance was not decreased by the amount transferred from her.");
            assertEq(preBobBalance + amount, cut.balanceOf(bob), "Bob's balance was not increased by the amount transferred to him.");
        }
    }
    ...
    /// @custom:ercx-concerned-function transferFrom
    function testFeeTakingTransferFromPresent(uint256 amount, uint256 balance1, uint256 balance2) 
    external {
    ...
    /// @custom:ercx-categories transferFrom, fees
    /// @custom:ercx-concerned-function transferFrom
    function testFeeTakingTransferFromPotential(uint256[2] memory feeAmounts, uint256 amount, uint256 balance1, uint256 balance2) 
    external initializeFees(feeAmounts) {
        _internalFeeTakingTransferFrom(amount, balance1, balance2);
    }
    ...
    ****************************/
    ...
    external initializeStateTwoUsers(balance1, balance2) {
        ...
        assertEq(preDifferentIdBalance, postDifferentIdBalance, "Some user different from the tokenSender and tokenReceiver had her balance modified after a transfer.");
    }
    ...
    external initializeStateTwoUsers(balance1, balance2) {
        ...
        CallResult memory callResultTransferFrom = _tryBobTransferFrom(alice, bob, amount);
        // Skip the test if the transferFrom call fails
        conditionalSkip(!callResultTransferFrom.success, "Inconclusive test: Alice could not transfer to Bob via `transferFrom` call.");        
        uint256 postDifferentIdBalance = cut.balanceOf(carol);
        assertEq(preDifferentIdBalance, postDifferentIdBalance, "Some user different from the tokenSender and tokenReceiver had her balance modified after a transferFrom.");
    }
    ...
    external initializeStateTwoUsers(balance1, balance2) {
        ...
        // Skip the test if the transferFrom call fails
        conditionalSkip(!callResultTransferFrom.success, "Inconclusive test: Bob could not transfer to Alice via a `transferFrom` call.");
        uint256 postTotalSupply = cut.totalSupply();
    ...
    external initializeStateTwoUsers(balance1, 0) {
        ...
        // Skip the test if the transferFrom call fails
        conditionalSkip(!callResultTransferFrom.success, "Inconclusive test: Bob could not transfer to Alice via a `transferFrom` call.");
        assertEq(allowance - amount, cut.allowance(alice, bob), "The allowance of Alice to Bob was not decreased by the transferred amount between them.");
    }
    ...
    external updateOwner() initializeStateTwoUsers(initialBalanceAlice, 0) {
    ...
    external initializeStateTwoUsers(initialBalanceAlice, 0) {
        uint256 prevBalanceAlice = cut.balanceOf(alice);
        ...
        assertEq(prevBalanceAlice - burnedAmount, cut.balanceOf(alice), "After a burn, the balance of the account from which tokens were burned was not updated correctly.");
    }
    ...
    external initializeStateTwoUsers(initialBalanceAlice, 0) {
        ...
        // Skip the test if the burn call fails
        conditionalSkip(!callResultBurn.success, "Inconclusive test: Failed to burn tokens.");
        assertEq(prevBalanceAlice, cut.balanceOf(alice), "After a burn of zero tokens, the balance of the account from which tokens were burned has changed.");
    }
    ...
    external initializeStateTwoUsers(initialBalanceAlice, 0) {
        ...
        // Skip the test if the burn call fails
        conditionalSkip(!callResultBurn.success, "Inconclusive test: Failed to burn tokens.");
        assertEq(initialTotalSupply - burnedAmount, cut.totalSupply(), "After a burn, the total supply was not updated correctly.");
    }
    ...
    external initializeStateTwoUsers(initialBalanceAlice, 0) {
        ...
        assertEq(initialUserBalanceAlice - burnedAmount, cut.balanceOf(alice), "After a burnFrom, the balance of the tokenApprover was not updated correctly.");
        assertEq(initialUserBalanceBob, cut.balanceOf(bob), "After a burnFrom, the balance of the tokenApprovee has changed.");
    }
    ...
    external initializeStateTwoUsers(initialBalanceAlice, 0) {
        ...
        // Skip the test if the burnFrom call fails
        conditionalSkip(!callResultBurn.success, "Inconclusive test: Failed to burnFrom tokens.");
        assertEq(initialUserBalance, cut.balanceOf(alice), "After a burnFrom of zero token, the balance of the tokenApprover has changed.");
        assertEq(initialUserBalanceBob, cut.balanceOf(bob), "After a burnFrom of zero token, the balance of the tokenApprovee has changed.");
    }
    ...
    external initializeStateTwoUsers(initialBalanceAlice, 0) {
        ...
        // Skip the test if the burnFrom call fails
        conditionalSkip(!callResultBurn.success, "Inconclusive test: Failed to burnFrom tokens.");
        assertEq(initialTotalSupply - burnedAmount, cut.totalSupply(), "After a burnFrom, the total supply was not updated correctly.");
    }
    ...
    external initializeStateTwoUsers(initialBalanceAlice, 0) {
        ...
        // Skip the test if the burnFrom call fails
        conditionalSkip(!callResultBurn.success, "Inconclusive test: Failed to burnFrom tokens.");
        assertEq(allowedAmount - burnedAmount, cut.allowance(alice, bob), "After a burnFrom, the allowance of the tokenApprovee for tokenApprover was not updated correctly.");
    }
    ...
    external initializeStateTwoUsers(initialBalanceAlice, 0) {
        ...
        // Skip the test if the burnFrom call fails
        conditionalSkip(!callResultBurn.success, "Inconclusive test: Failed to burnFrom tokens.");
        assertEq(allowedAmount, cut.allowance(alice, bob), "After a burnFrom of zero token, the allowance of the tokenApprovee for tokenApprover has changed.");
    }
    ...
    function testMintingUpdatesBalance(uint256 mintedAmount) 
    external updateOwner() {
        uint256 initialTotalSupply = cut.totalSupply();
        ...
        assertEq(initialUserBalance + mintedAmount, cut.balanceOf(alice), "Alice's balance was not augmented with the amount minted to her.");
    }
    ...
    /// @custom:ercx-categories balance, mint
    /// @custom:ercx-concerned-function mint, mintToken, issue
    function testMintingUpdatesTotalSupply(uint256 mintedAmount) 
    external updateOwner() {
        uint256 initialTotalSupply = cut.totalSupply();
        vm.assume(mintedAmount > 0);
        vm.assume(mintedAmount < MAX_UINT256 - initialTotalSupply);
        ...
        // Skip the test if the mint call fails
        conditionalSkip(!callResultMint.success, "Inconclusive test: Failed to mint tokens.");
        assertEq(initialTotalSupply + mintedAmount, cut.totalSupply(), "The total supply was not augmented by the minted amount.");
    }
    ...
    /// @custom:ercx-concerned-function mint, mintToken, issue
    function testMintingToZeroAddressShouldFail(uint256 mintedAmount) 
    external updateOwner() {
        vm.assume(mintedAmount > 0);
        vm.assume(mintedAmount < MAX_UINT256 - cut.totalSupply());
        assertFail(_mintTokenToAddress(mintedAmount, address(0x0)), "It was possible to mint tokens to the zero address.");
    }
    ...
    /// @custom:ercx-categories mint, zero address
    /// @custom:ercx-concerned-function mint, mintToken, issue
    function testMintingZeroShouldNotChangeTotalSupply() 
    external updateOwner() {
        uint256 initialTotalSupply = cut.totalSupply();
        CallResult memory callResultMint = _mintTokenToAlice(0);
        // Skip the test if the mint call fails
        conditionalSkip(!callResultMint.success, "Inconclusive test: Failed to mint tokens.");
        assertEq(initialTotalSupply, cut.totalSupply(), "Total supply has changed unexpectedely while minting zero token to an account.");
    }
    ...
    /// @custom:ercx-categories mint, zero address
    /// @custom:ercx-concerned-function mint, mintToken, issue
    function testMintingZeroShouldNotChangeBalance() 
    external updateOwner() {
        uint256 initialUserBalance = cut.balanceOf(alice);
        CallResult memory callResultMint = _mintTokenToAlice(0);
        // Skip the test if the mint call fails
        conditionalSkip(!callResultMint.success, "Inconclusive test: Failed to mint tokens.");
        assertEq(initialUserBalance, cut.balanceOf(alice), "Balance of Alice has changed unexpectedely while minting zero token to this address.");
    }
    ...
    function testTotalSupplyDoesNotOverflowByMinting(uint256 amountToMint) external
    updateOwner() unpauseIfPaused() {
        ...
        if (callResultMint.success) {
            ...
            assertTrue((newTotalSupply == MAX_UINT256) || (newTotalSupply == totalSupply_), "The total supply has not been augmented by the amount minted.");
        }
    }
    ...
    function testBalanceDoesNotOverflowByMinting(uint256 initialBalance) external
    updateOwner() initializeStateTwoUsers(initialBalance, initialBalance) {
        ...
        if (callResultMint.success) {
            ...
            if (callResultMint1.success) {
                ...
                assertLt(prevBalance, nextBalance, "After minting some tokens to Alice, her balance has not been augmented.");
            }
        }
    }
    ...
    function testNoExcessByMintingViaOverflow(uint256 amountToMint) external
    updateOwner() {
    ...
    function testZeroIncreaseAllowanceEventEmission() 
    external {
        vm.expectEmit(true, true, true, true);
        emit Approval(alice, bob, 0);
        assertSuccess(_tryCustomerIncreaseAllowance(alice, bob, 0), "Calling increaseAllowance of zero amount did not emit the Approval event.");
    }
    ...
    /// @custom:ercx-concerned-function increaseAllowance, Approval
    function testPositiveIncreaseAllowanceEventEmission(uint256 amount) 
    external {
        vm.assume(amount > 0);
        vm.expectEmit(true, true, true, true);
        emit Approval(alice, bob, amount);
        assertSuccess(_tryCustomerIncreaseAllowance(alice, bob, amount), "Calling increaseAllowance of zero amount did not emit the Approval event.");
    }
    ...
    function testPositiveIncreaseAllowanceAllowsZeroTransferFrom(uint256 increaseAmount, uint256 balance1, uint256 balance2, uint256 initAllowance) 
    external {
    ...
    /// @custom:ercx-concerned-function increaseAllowance
    function testPositiveIncreaseAllowanceAllowsPositiveTransferFrom(uint256 increaseAmount, uint256 transferAmount, uint256 balance1, uint256 balance2, uint256 initAllowance) 
    external {
        vm.assume(transferAmount > 0);
        internalTestPositiveIncreaseAllowanceAllowsTransferFrom(increaseAmount, transferAmount, balance1, balance2, initAllowance);
    }
    ...
    function internalTestPositiveIncreaseAllowanceAllowsTransferFrom(uint256 increaseAmount, uint256 transferAmount, uint256 balance1, uint256 balance2, uint256 initAllowance) 
    internal initializeAllowanceOneUser(initAllowance) initializeStateTwoUsers(balance1, balance2) {
        ...
        assertSuccess(_tryBobTransferFrom(alice, bob, transferAmount), "Bob could not transferFrom Alice after she allowed Bob with increaseAllowance.");
    }
    ...
    external initializeStateTwoUsers(balance1, 0) {
        ...
        assertEq(allowance + addedValue, cut.allowance(alice, bob), "Alice allowance to Bob was not increased by the amount used with increaseAllowance.");
    }
    ...
    function testIncreaseAllowanceDoubleSpend(uint256 prevAllowance, uint256 increaseAmount, uint256 balance1)
        external initializeStateTwoUsers(balance1, 0) initializeAllowanceOneUser(prevAllowance){
        ...
        assertEq(cut.allowance(alice, bob), allowanceLeft + increaseAmount, "Alice's increased allowance to Bob was not reflected correctly.");
    }
    ...
    /// @custom:ercx-concerned-function increaseAllowance
    function testZeroSelfIncreaseAllowance(uint256 balance1, uint256 balance2)
    external {
    ...
    /// @custom:ercx-categories allowance
    /// @custom:ercx-concerned-function increaseAllowance
    function testPositiveSelfIncreaseAllowance(uint256 amount, uint256 balance1, uint256 balance2)
    external {
        vm.assume(amount > 0);
    ...
    internal initializeStateTwoUsers(balance1, balance2) {
        ...
        assertEq(cut.allowance(alice, alice), amount, "Alice's allowance to herself was not taken into account after self approving.");
    }
    ...
    /// @custom:ercx-concerned-function increaseAllowance
    function testZeroSelfIncreaseAllowanceTransferFrom(uint256 balance1, uint256 balance2)
    external {
    ...
    internal initializeStateTwoUsers(balance1, balance2) {
        ...
        assertSuccess(_tryAliceIncreaseAllowance(alice, amount));
        // Alice transfers using transferFrom
        assertSuccess(_tryAliceTransferFrom(alice, bob, amount), "After Alice self-approved, she could not transferFrom to Bob.");
    }
    ...
    function testZeroDecreaseAllowanceEventEmission() 
    external {
        vm.expectEmit(true, true, true, true);
        emit Approval(alice, bob, 0);
        assertSuccess(_tryCustomerDecreaseAllowance(alice, bob, 0), "After reducing the allowance by 0 with decreaseAllowance, the Approval event was not emitted.");
    }
    ...
    external initializeAllowanceOneUser(allowance){
        vm.assume(amount > 0);
        vm.assume(amount <= allowance);
        vm.expectEmit(true, true, true, true);
        emit Approval(alice, bob, allowance - amount);
        assertSuccess(_tryAliceDecreaseAllowance(bob, amount), "After reducing the allowance with decreaseAllowance, the Approval event was not emitted.");
    }
    ...
    function testPositiveDecreaseAllowanceAllowsZeroTransferFrom(uint256 initAllowance, uint256 decreaseAmount, uint256 balance1, uint256 balance2) 
    external {
    ...
    /// @custom:ercx-categories allowance, transferFrom, balance
    /// @custom:ercx-concerned-function decreaseAllowance
    function testPositiveDecreaseAllowanceAllowsPositiveTransferFrom(uint256 initAllowance, uint256 decreaseAmount, uint256 transferAmount, uint256 balance1, uint256 balance2) 
    external {
        vm.assume(transferAmount > 0);
        internalTestPositiveDecreaseAllowanceAllowsTransferFrom(initAllowance, decreaseAmount, transferAmount, balance1, balance2);
    }
    ...
    /// the said amount MUST be transferable by tokenApprovee, provided a sufficient balance of tokenApprover.
    function internalTestPositiveDecreaseAllowanceAllowsTransferFrom(uint256 initAllowance, uint256 decreaseAmount, uint256 transferAmount, uint256 balance1, uint256 balance2) 
    internal initializeAllowanceOneUser(initAllowance) initializeStateTwoUsers(balance1, balance2) {
        ...
        assertSuccess(_tryBobTransferFrom(alice, bob, transferAmount), "After Alice allowance to Bob was decreased, Bob could not transferFrom some amount up to the remaining allowance.");
    }
    ...
    external initializeStateTwoUsers(balance1, 0) {
        ...
        assertEq(allowance - substractedValue, cut.allowance(alice, bob), "Bob's reduced allowance with decreaseAllowance was not reflected in his allowance.");
    }
    ...
    /// @custom:ercx-categories allowance
    /// @custom:ercx-concerned-function decreaseAllowance
    function testDecreaseAllowanceDoubleSpend(uint256 prevAllowance, uint256 decreaseAmount, uint256 balance1, uint256 balance2, uint256 transferAmount)
    external initializeAllowanceOneUser(prevAllowance) initializeStateTwoUsers(balance1, balance2){
        ...
        assertEq(cut.allowance(alice, bob), allowanceLeft - decreaseAmount, "Alice's decreased allowance to Bob was not reflected correctly.");
    }
    ...
    /// @custom:ercx-categories allowance
    /// @custom:ercx-concerned-function decreaseAllowance
    function testDecreaseAllowanceBehaviorExact(uint256 prevAllowance, uint256 decreaseAmount, uint256 balance1, uint256 balance2)
    external initializeAllowanceOneUser(prevAllowance) initializeStateTwoUsers(balance1, balance2){
        vm.assume(prevAllowance < decreaseAmount);
        // Alice tries to decrease more allowance from Bob to what he has
        assertFail(_tryAliceDecreaseAllowance(bob, decreaseAmount), "A call to decreaseAllowance was succesful even though there was no allowance to reduce.");
    }
    ...
    /// @custom:ercx-categories allowance, zero amount
    /// @custom:ercx-concerned-function decreaseAllowance
    function testZeroSelfDecreaseAllowance(uint256 initAllowance, uint256 balance1, uint256 balance2)
    external {
    ...
    /// @custom:ercx-categories allowance
    /// @custom:ercx-concerned-function decreaseAllowance
    function testPositiveSelfDecreaseAllowance(uint256 initAllowance, uint256 decreaseAmount, uint256 balance1, uint256 balance2)
    external {
        vm.assume(decreaseAmount > 0);
        _internalTestSelfDecreaseAllowance(initAllowance, decreaseAmount, balance1, balance2);
    }

    function _internalTestSelfDecreaseAllowance(uint256 initAllowance, uint256 decreaseAmount, uint256 balance1, uint256 balance2)
    internal initializeAllowanceSelf(initAllowance) initializeStateTwoUsers(balance1, balance2) {
        ...
        assertEq(cut.allowance(alice, alice), initAllowance - decreaseAmount, "Alice decreased allowance to herself was not reflected correctly.");    
    }
    ...
    /// @custom:ercx-categories allowance, zero amount, transferFrom
    /// @custom:ercx-concerned-function decreaseAllowance
    function testZeroSelfDecreaseAllowanceTransferFrom(uint256 initAllowance, uint256 balance1, uint256 balance2, uint256 transferAmount)
    external {
    ...
    /// @custom:ercx-categories allowance, transferFrom
    /// @custom:ercx-concerned-function decreaseAllowance
    function testPositiveSelfDecreaseAllowanceTransferFrom(uint256 initAllowance, uint256 amount, uint256 balance1, uint256 balance2, uint256 transferAmount)
    external {
        vm.assume(amount > 0);
        _internalTestSelfDecreaseAllowanceTransferFrom(initAllowance, amount, balance1, balance2, transferAmount);
    }

    function _internalTestSelfDecreaseAllowanceTransferFrom(uint256 initAllowance, uint256 decreaseAmount, uint256 balance1, uint256 balance2, uint256 transferAmount)
    internal initializeAllowanceSelf(initAllowance) initializeStateTwoUsers(balance1, balance2) {
        vm.assume(decreaseAmount < cut.allowance(alice, alice));
        ...
        assertSuccess(_tryAliceDecreaseAllowance(alice, decreaseAmount));
        // Alice transfers using transferFrom
        assertSuccess(_tryAliceTransferFrom(alice, bob, transferAmount), "Alice could not transferFrom her account after decreasing her allowance.");
    }
    ...
    ***********************************************************************************/
    ...
    function testNonWalletAddressCannotEnableTransfer() external {
        ...
        conditionalSkip(!successWallet, "Inconclusive test: Impossible to retrieve wallet address using variable walletAddress.");
    ...
    function testNonWalletAddressCannotDisableTransfer() external {
        ...
        address walletAddress = abi.decode(data, (address));
        vm.assume(alice != walletAddress);
        assertFail(_tryDisableTokenTransfer(alice), "Alice, who is not the walletAddress, could disable token transfers.");
    }
    ...
    ***********************************************************************************/
    ...
    function testNonOwnerCannotChangeOwner() external updateOwner() {
        ...
        if (successSetOwner) {
            ...
            if (contractOwner == bob || contractOwner != initialContractOwer) {
                assertTrue(false, "Alice, who is not the contract owner, was able to change the registered contract owner.");
            }
        }
        else {
    ...
    function testOwnerCanChangeOwner() external updateOwner() {
        ...
        _updateContractOwner();
        assertTrue(contractOwner == bob && contractOwner != initialContractOwner, "The contract owner was not able to change the registered contract owner.");
    }
    ...
    ***********************************************************************************/
    ...
    function testBatchTransferDoesNotOverflow(uint256 balanceAlice, uint256 transferAmount) external
    initializeStateTwoUsers(balanceAlice, 0) setupReceivers() updateOwner() unpauseIfPaused() {
    ...
    /// @custom:ercx-concerned-function batchTransfer
    /// @custom:ercx-note from Awesome Buggy A1
    function testBatchTransferDoesNotOverflowByContractOwner(uint256 transferAmount) external
    setupReceivers() updateOwner() unpauseIfPaused() {
    ...
    ***********************************************************************************/
    ...
    function testOwnerCannotOverflowSellBySettingPrice(uint256 amount, uint256 balanceAlice) external
    initializeStateTwoUsers(balanceAlice, 0) updateOwner() {
        vm.assume(amount <= cut.balanceOf(alice));
        vm.assume(amount > 1);
        ...
        if (successSetPrices) {
            ...
            if (result.success) {
                ...
                assertLt(alicePreSellBalanceEth + sellPrice, alicePostSellBalanceEth, "Alice ether balance was not augmented with the corresponding amount from the sold tokens."); // Since Alice is selling strictly more than one token, she should have strictly more than her previous balance + the price in ether of one token
            }
        }
    }
