...
abstract contract ERCAbstract is Test {
        ...
        returns (bool success, string memory reason)
    {
        try this.externalDeal(token, tokenReceiver, amount, true) {
            if (_tryERC20TokenBalanceOfUser(token, tokenReceiver) == amount) {
                ...
                (bool transferSuccess, string memory failReason) =
                    _tryTopERC20TokenHolderTransferToReceiver(token, tokenReceiver, amount);
        ...
        returns (bool, string memory)
    {
        ...
        if (topTokenHolder != address(0x0)) {
            ...
            if (_tryERC20TokenBalanceOfUser(token, tokenReceiver) >= lowerBoundedAmount) {
                ...
                return (
                    false,
                    "Top token holder failed to deal at least 80% of the intended amount of tokens to dummy users."
                );
            }
        } else {
    ...
    function _tryERC20TokenBalanceOfUser(address token, address user) internal returns (uint256) {
    ...
    function _callOptionalReturn(address called_address, bytes memory data) internal returns (CallResult memory) {
        (bool success, bytes memory returndata) = called_address.call(data);
        if (success && returndata.length > 0) {
            bool returnValue = abi.decode(returndata, (bool));
            OptionalReturn optionalReturn = returnValue ? OptionalReturn.RETURN_TRUE : OptionalReturn.RETURN_FALSE;
            return CallResult(success, optionalReturn);
        }
        return CallResult(success, OptionalReturn.RETURN_ABSENT);
    }

    function assertSuccess(CallResult memory result) internal {
        assertTrue(result.success && result.optionalReturn != OptionalReturn.RETURN_FALSE);
    }

    function assertSuccess(CallResult memory result, string memory err) internal {
        assertTrue(result.success && result.optionalReturn != OptionalReturn.RETURN_FALSE, err);
    }

    function assertFail(CallResult memory result) internal {
        assertFalse(result.success && result.optionalReturn != OptionalReturn.RETURN_FALSE);
    }

    function assertFail(CallResult memory result, string memory err) internal {
