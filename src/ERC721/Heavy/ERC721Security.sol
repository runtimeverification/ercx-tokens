...
abstract contract ERC721Security is ERC721Abstract {
    ...
    ****************************/
    ...
    function testSafeTransferFromWithDataToEOAByOwnerUpdatesBalances(address tokenReceiver, uint256 tokenId, bytes memory data)
    ...
    /// @custom:ercx-categories transfer, balance
    /// @custom:ercx-concerned-function safeTransferFrom, balanceOf
    function testSafeTransferFromWithDataByReceiverOwnerToHimSelfDoesNotUpdatesBalance(uint256 tokenId, bytes memory data)
    external withUsers() dealAnOwnedTokenToAlice(tokenId) {
    ...
    /// @custom:ercx-categories transfer, balance
    /// @custom:ercx-concerned-function safeTransferFrom, balanceOf
    function testSafeTransferFromWithDataByEOAOwnerToHimSelfDoesNotUpdatesBalance(address owner, uint256 tokenId, bytes memory data)
    ...
    /// @custom:ercx-categories transfer, balance
    /// @custom:ercx-concerned-function safeTransferFrom, balanceOf
    function testSafeTransferFromWithDataByApprovedAddressToOwnerDoesNotUpdatesBalance(address approvedAddress, uint256 tokenId, bytes memory data)
    external withUsers() dealAnOwnedTokenToAlice(tokenId) {
        ...
        CallResult memory callTransfer = _tryCustomerSafeTransferFromWithData(approvedAddress, alice, alice, tokenId, data);
    ...
    function testSafeTransferFromWithDataByApprovedEOAAddressToSelfUpdatesBalances(address approved, uint256 tokenId, bytes memory data)
    ...
    /// @custom:ercx-categories transfer, balance, approval
    /// @custom:ercx-concerned-function safeTransferFrom, balanceOf, approve
    function testSafeTransferFromWithDataByApprovedReceiverAddressToSelfUpdatesBalances(uint256 tokenId, bytes memory data)
    external withUsers() dealAnOwnedTokenToAlice(tokenId) {
    ...
    /// @custom:ercx-categories transfer, balance, approval
    /// @custom:ercx-concerned-function safeTransferFrom, balanceOf, approve
    function testSafeTransferFromWithDataByApprovedAddressToEOAUpdatesBalances(address tokenReceiver, uint256 tokenId, bytes memory data)
    external withUsers() ensureNotATokenReceiver(tokenReceiver) dealAnOwnedTokenToAlice(tokenId) {
        vm.assume(tokenReceiver != address(0x0));
    ...
    /// @custom:ercx-categories transfer, balance, approval
    /// @custom:ercx-concerned-function safeTransferFrom, balanceOf, approve
    function testSafeTransferFromWithDataByOperatorToEOAUpdatesBalances(address tokenReceiver, uint256 tokenId, bytes memory data)
    external withUsers() ensureNotATokenReceiver(tokenReceiver) dealAnOwnedTokenToAlice(tokenId) {
        vm.assume(tokenReceiver != address(0x0));
    ...
    ****************************/
    ...
    external withUsers() ensureNotATokenReceiver(owner) dealAnOwnedTokenToCustomer(owner, tokenId) {
        uint256 ownerInitialBalance = cut.balanceOf(owner);
        CallResult memory callTransfer = _tryCustomerSafeTransferFrom(owner, owner, owner, tokenId);
        conditionalSkip(!callTransfer.success, "Inconclusive test: Could not perform safeTransferFrom from Alice to self.");
        assertEq(cut.balanceOf(owner), ownerInitialBalance, "Balance of a user has changed after sending a token to himself.");
    }
    ...
    external withUsers() ensureNotATokenReceiver(approvedAddress) dealAnOwnedTokenToAlice(tokenId) {
        vm.assume(approvedAddress != alice); // msg.sender cannot approve herself in some contracts
        uint256 aliceInitialBalance = cut.balanceOf(alice);
        uint256 approvedInitialBalance = cut.balanceOf(approvedAddress);
        CallResult memory callApprove = _tryAliceApprove(approvedAddress, tokenId);
        ...
        conditionalSkip(!callTransfer.success, "Inconclusive test: approved address could not safeTransferFrom from Alice to him.");
        assertEq(cut.balanceOf(alice), aliceInitialBalance - 1, "Balance of a user has not been decremented after sending a token.");
        assertEq(cut.balanceOf(approvedAddress), approvedInitialBalance + 1, "Balance of a user has not been incremented after receiving a token.");
    }
